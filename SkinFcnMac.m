function [CiBW,CiE,CsW,CsE] = SkinFcnMac(t,u, Global)
...
tmin = t/60; 
thor = t/3600;
% ---------- Configuraci�n - Vector de salida -----------------------------
...
% -------------------------------------------------------------------------
u8b = zeros(index1,index2); u8e = zeros(index1,index2);  
% -------------------------------------------------------------------------
    for j=1:index1 
        ...
        for i=1:index2, u8b(j,i)=u(j,i+14*index2);    end
        for i=1:index2, u8e(j,i)=u(j,i+15*index2);    end
    end
% ----- Concentraci�n - Especies (GAS) Burbuja & Estela [mmol/cm3] --------
  CiBW(:,:,1) = u1b; CiBW(:,:,2) = u2b; CiBW(:,:,3) = u3b;
  CiBW(:,:,4) = u4b; CiBW(:,:,5) = u5b; CiBW(:,:,6) = u6b;
% ----- Concentraci�n - Especies (GAS) Emulsi�n [mmol/cm3] ----------------
   CiE(:,:,1) = u1e; CiE(:,:,2) = u2e; CiE(:,:,3) = u3e;
   CiE(:,:,4) = u4e; CiE(:,:,5) = u5e; CiE(:,:,6) = u6e;
% -- Concentraci�n - Especies (S�LIDO) Estela & Emulsi�n [g.coque/g.cat] --
          CsW = u7w;             CsE = u7e;
% -------------------------------------------------------------------------
...
% --------------------------| FIGURA - SET MAC |---------------------------
 ...
 LFZ = 5; % LEYNEDA
% --------------------------| FASE GAS BURBUJA |---------------------------
% ---------- Gr�fico - Concentraci�n vs tiempo ----------------------------
  ...
  close all
% ---------- Gr�fico - Concentraci�n vs espacio ---------------------------
   id = exist('CONCENTRACIONES','file');
            if id == 7
  ...
  for ii = 1:index3
    fig2 = figure;
          set(fig2,'Units','centimeters',...
                   'PaperPosition',[0 0 15 15],...
                   'PaperSize', [15,15]);
    ax2 = axes('Parent',fig2,'FontSize',FZ1,'XGrid','off',...
            'YGrid','off','visible','on','Box', 'on',...
            'TickLabelInterpreter','latex');
          set(fig2, 'Color', 'w') 
    hold on
...
% ---------- Gr�fico - Concentraci�n vs tiempo ----------------------------
 id = exist('CONCENTRACIONES','file');
            if id == 7
               dir = strcat(pwd,'/',TAG5{1},'/',TAG3{1},TAG4{3});
            else
                mkdir('CONCENTRACIONES')
               dir = strcat(pwd,'/',TAG5{1},'/',TAG3{1},TAG4{3});
            end
%   for ii = 1:index3
   fig1 = figure;
    ...
    hold on
    for i = 1:index2
            plot(tmin,CsW(:,i),'ko-','MarkerSize',MZ1);
    end 
    xlabel('$tiempo\;\left( {min} \right)$',...
           'FontSize',XLFZ,'interpreter','Latex')
    ylabel(TAG1(7),...
...
id = exist('CONCENTRACIONES','file');
            if id == 7
               dir = strcat(pwd,'/',TAG5{1},'/',TAG3{2},TAG4{3});
            else
                mkdir('CONCENTRACIONES')
               dir = strcat(pwd,'/',TAG5{1},'/',TAG3{2},TAG4{3});
            end
%   for ii = 1:index3
   fig2 = figure;
    ...
    hold on
    for i = 1:index1
            plot(zg,CsW(i,:)','ko-','MarkerSize',MZ1);
    end   
    xlabel('$z\left( {cm} \right)$',...
           'FontSize',XLFZ,'interpreter','Latex')
    ylabel(TAG1(7),...
...
%   for ii = 1:index3
   fig1 = figure;
    ...
    hold on
    for i = 1:index2
            plot(tmin,CsE(:,i),'ko-','MarkerSize',MZ1);
    end 
    xlabel('$tiempo\;\left( {min} \right)$',...
           'FontSize',XLFZ,'interpreter','Latex')
    ylabel(TAG1(7),...
...
%   for ii = 1:index3
    ...
    hold on
    for i = 1:index1
            plot(zg,CsE(i,:)','ko-','MarkerSize',MZ1);
    end   
    xlabel('$z\left( {cm} \right)$',...
           'FontSize',XLFZ,'interpreter','Latex')
    ylabel(TAG1(7),...
...
%   end
% xxxx
% ---------- Gr�fico Espacial a tiempo final ------------------------------
% --------------------------| FASE GAS BURBUJA |---------------------------
[m,~,~] = size(CiBW);
 id = exist('CONCENTRACIONES','file');
            if id == 7
    ...
    hold on
            ...
            plot(zg,CiBW(m,:,6)','k--','MarkerSize',MZ1);
...
[m,~,~] = size(CiE);
 id = exist('CONCENTRACIONES','file');
    ...
    hold on
            ...
            plot(zg,CiE(m,:,6)','k--','MarkerSize',MZ1);
    ley1 = {'$C{H_4}$','$C{O_2}$','$CO$','${H_2}$','${H_2}O$','${N_2}$'};
    legend(ley1,'Interpreter','Latex',     ...
                'Location','north',        ...
                'Orientation','horizontal',...
    ...
    print(fig2,'-dpdf','-r500',dir)
  close all
% ---------- Gr�fico Temporal - salida del reactor ------------------------
% --------------------------| FASE GAS BURBUJA |---------------------------
[~,n,~] = size(CiBW);
 id = exist('CONCENTRACIONES','file');
            if id == 7
    ...
    hold on
            ...
            plot(tmin,CiBW(:,n,6)','k--','MarkerSize',MZ1);
    ley1 = {'$C{H_4}$','$C{O_2}$','$CO$','${H_2}$','${H_2}O$','${N_2}$'};
    legend(ley1,'Interpreter','Latex',     ...
                'Location','north',        ...
                'Orientation','horizontal',...
...
% --------------------------| FASE GAS EMULSI�N |--------------------------  
 ...
 id = exist('CONCENTRACIONES','file');
    ...
    hold on
            ...
            plot(tmin,CiE(:,n,6)','k--','MarkerSize',MZ1);
    ley1 = {'$C{H_4}$','$C{O_2}$','$CO$','${H_2}$','${H_2}O$','${N_2}$'};
    legend(ley1,'Interpreter','Latex',     ...
                'Location','north',        ...
                'Orientation','horizontal',...
