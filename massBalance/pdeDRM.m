function ut = pdeDRM(t,u)
% Función pdeDRM.m define las EDOs en la solución numérica del modelo 
% mediante el método de las lineas (M.O.L).
% ----------------------------| ENTRADAS |---------------------------------
%       t = vector con las salidas de tiempo a evaluar
%       u = vector con los valores iniciales de cada variable dependiente
% ----------------------------| SALIDAS |----------------------------------
%      ut = vector con la variación temporal de cada variable dependiente
% -------------------------------------------------------------------------
  global ncall zg fw Emf CH4in CO2in N2in Num_esp Dcat CcMax
% ---------- Configuración inicial de las variables -----------------------
  index1 = length(zg);
% ---------- Eliminación de concentraciones negativas ---------------------
...
% ----------------------------| FASE GAS |---------------------------------
  ...
  for i = 1:(index1), u1e(i) = u(i+6*(index1));end  % u1e = CH4 emulsión
  for i = 1:(index1), u2e(i) = u(i+7*(index1));end  % u2e = CO2 emulsión
  for i = 1:(index1), u3e(i) = u(i+8*(index1));end  % u3e = CO  emulsión
  for i = 1:(index1), u4e(i) = u(i+9*(index1));end  % u4e = H2  emulsión
  for i = 1:(index1), u5e(i) = u(i+10*(index1));end % u5e = H2O emulsión
  for i = 1:(index1), u6e(i) = u(i+11*(index1));end % u6e = N2  emulsión
  for i = 1:(index1), u7w(i) = u(i+12*(index1));end % u7w = Cc  estela
  for i = 1:(index1), u7e(i) = u(i+13*(index1));end % u7e = Cc  emulsión
% -------------------------------------------------------------------------
%    if ncall > 435
%        for i = 1:index1
